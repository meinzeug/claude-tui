...
class AuthenticationMiddleware(BaseHTTPMiddleware):
    ...
    async def _authenticate_request(self, request: Request) -> Optional[Dict[str, Any]]:
        ...
        # Get user from database using token_payload.sub
        try:
            # Import user repository here to avoid circular imports
            ...
            async with get_async_session() as session:
                ...
                if not user:
                    raise AuthenticationError(f"User not found: {token_payload.sub}")
                
                if not user.is_active:
                    raise AuthenticationError("User account is inactive")
                    
        except ImportError as e:
            logger.warning(f"Database not available, using fallback user object: {e}")
            # Fallback to mock user object if database is not available
            user = type('User', (), {
