...
class TaskDashboard(Vertical):
    ...
    async def _create_new_task(self) -> None:
        ...
        try:
            ...
            self.notify(f"Failed to create task due to connection issue: {e}", severity="error")
        except (KeyError, ValueError) as e:
            # Handle validation/data errors
            self.notify(f"Invalid task data: {e}", severity="warning")
        except Exception as e:
            # Handle unexpected errors with logging
            self.notify(f"Unexpected error creating task: {e}", severity="error")
    ...
    def refresh(self, *, repaint: bool = True, layout: bool = False, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # Continue without logging if logger module unavailable
    ...
    async def _force_refresh(self) -> None:
        ...
        try:
            ...
            # Handle backend connection issues silently in background refresh
            pass  # Expected in offline mode
        except (AttributeError, KeyError) as e:
            # Handle malformed backend response
            pass  # Backend may return incomplete data
        except Exception as e:
            # Log unexpected errors for debugging
            import logging
            logging.getLogger(__name__).debug(f"Background refresh error: {e}")
