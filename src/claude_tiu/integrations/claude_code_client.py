"""Claude Code Client - Production HTTP API integration with Claude Code.

Provides OAuth authentication, HTTP client with rate limiting, retry logic,
and robust error handling for seamless integration with Claude Code API.
"""
...
from claude_tiu.models.ai_models import CodeContext, CodeResult, ReviewCriteria, CodeReview
from claude_tiu.utils.security import SecurityManager
...
class ClaudeCodeClient:
    ...
    def __init__(
        ...
        base_url: str = "https://api.claude.ai/v1",
        ...
        Args:
            ...
            base_url: Claude Code API base URL
        ...
        # Authentication
        self.oauth_token = oauth_token or config_manager.get('CLAUDE_CODE_OAUTH_TOKEN')
        if not self.oauth_token:
            logger.warning("No OAuth token provided. API calls will fail.")
        ...
        logger.info(f"Claude Code API client initialized - Base URL: {self.base_url}")
    
    async def execute_task(self, task_description: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Execute a coding task using Claude Code API.
        ...
        request = TaskRequest(
            ...
            model=context.get('model', 'claude-3-sonnet') if context else 'claude-3-sonnet'
        )
        ...
        try:
            response = await self._make_request(
                ...
                timeout=request.timeout + 30  # Add buffer time
            )
    ...
    async def _ensure_auth(self) -> None:
        ...
        """
        if not self.oauth_token:
            raise ClaudeCodeAuthError("No OAuth token available. Set CLAUDE_CODE_OAUTH_TOKEN environment variable.")
        ...
        async with self._token_lock:
            try:
                ...
                refresh_data = {
                    ...
                    'client_id': self.config_manager.get('CLAUDE_CODE_CLIENT_ID'),
                    'client_secret': self.config_manager.get('CLAUDE_CODE_CLIENT_SECRET')
                }
    ...
    @staticmethod
    def create_with_token(oauth_token: str, base_url: str = "https://api.claude.ai/v1") -> 'ClaudeCodeClient':
        """
