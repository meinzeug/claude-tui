...
def init(
    ...
    """
    Initialize a new Claude-TIU project.
    
    Creates a new project with the specified template and configuration.
    
    Examples:
        claude-tiu core init my-app --template=web-app
        claude-tiu core init api-service --git --ai-features
        claude-tiu core init --interactive
    """
...
def build(
    ...
    Examples:
        claude-tiu core build --production
        claude-tiu core build --watch --verbose
        claude-tiu core build --clean --parallel=4
    """
...
def test(
    ...
    Examples:
        claude-tiu core test --coverage
        claude-tiu core test --filter="test_auth*" --parallel
        claude-tiu core test --integration --performance
    """
...
def deploy(
    ...
    Examples:
        claude-tiu core deploy --environment=production
        claude-tiu core deploy --dry-run --health-check
        claude-tiu core deploy --rollback=v1.2.3
    """
...
def validate(
    ...
    Examples:
        claude-tiu core validate --comprehensive
        claude-tiu core validate --fix --threshold=0.9
        claude-tiu core validate --report=validation_report.json
    """
...
async def init_project(
    ...
    try:
        ...
        panel = Panel.fit(
            ...
            f"  • cd {project_path}\n"
            f"  • claude-tiu core build\n"
            f"  • claude-tiu core test",
...
async def _setup_ai_features(project_path: Path, config_manager: ConfigManager) -> None:
    ...
    ai_config_path = project_path / ".claude-tiu" / "ai-config.json"
