#!/usr/bin/env python3
"""
Claude-TIU Enhanced CLI Main Entry Point.
...
def cli(
    ...
    Examples:
        claude-tiu                      # Launch TUI interface
        claude-tiu init my-project      # Initialize new project  
        claude-tiu ai generate --help   # AI code generation help
        claude-tiu workspace list       # List workspaces
        claude-tiu build --watch        # Build with file watching
    """
    if version:
        _show_version_info()
        sys.exit(0)
...
def _show_version_info() -> None:
    ...
    table = Table(title="Claude-TIU Version Information", show_header=False)
    table.add_column("Component", style="cyan", no_wrap=True)
    table.add_column("Version", style="green")
    
    table.add_row("Claude-TIU", __version__)
...
def main() -> None:
    """
    Main entry point for the claude-tiu CLI application.
    """
    cli()
