...
class LRUCache:
    ...
    def __init__(self, max_size: int = 1000, max_memory_mb: int = 100, cleanup_interval: int = 300):
    ...
    def _start_cleanup_task(self):
        ...
        try:
            ...
            # No event loop running yet, cleanup will be triggered on access
            pass
    ...
    async def _cleanup_expired(self):
        ...
        try:
            ...
            # Update memory stats
            self.stats.memory_usage_bytes = current_memory
            self.stats.total_entries = len(self.cache)
            
            # Enforce memory limit by removing LRU entries
            if current_memory > self.max_memory_bytes and self.cache:
                target_memory = self.max_memory_bytes * 0.8  # Clean to 80%
                while current_memory > target_memory and self.cache:
            ...
            if expired_keys or current_memory > self.max_memory_bytes:
                logger.info(
                    f"Cache cleanup: removed {len(expired_keys)} expired entries, "
                    f"memory: {current_memory / 1024 / 1024:.2f}MB, "
...
class AICache:
    ...
    def __init__(
        ...
        # Initialize cache levels with cleanup
        self.memory_cache = LRUCache(memory_cache_size, memory_limit_mb, cleanup_interval=300)
