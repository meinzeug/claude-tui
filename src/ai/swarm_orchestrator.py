...
class SwarmOrchestrator:
    ...
    async def _allocate_optimal_agents(self, swarm_id: str, task_request: TaskRequest) -> List[str]:
        """Intelligently allocate optimal agents for task execution"""
        available_agents = self.swarm_manager.swarm_agents.get(swarm_id, [])
        ...
        for agent in available_agents:
            ...
            # Prefer agents with relevant recent experience
            if hasattr(agent, 'recent_tasks'):
                for recent_task in getattr(agent, 'recent_tasks', [])[-5:]:
                    if any(keyword in recent_task.lower() for keyword in task_request.description.lower().split()):
                        score += 3
        ...
        optimal_count = min(
            max(1, task_request.estimated_complexity // 3),
            len(agent_scores),
            5  # Maximum agents per task
        )
        
        selected_agents = [agent_id for agent_id, score in agent_scores[:optimal_count] if score > 0]
        
        # Mark agents as busy
        for agent_id in selected_agents:
            await self._mark_agent_busy(swarm_id, agent_id)
        
        return selected_agents
    
    async def _mark_agent_busy(self, swarm_id: str, agent_id: str):
        """Mark agent as busy"""
        agents = self.swarm_manager.swarm_agents.get(swarm_id, [])
        for agent in agents:
            if agent.agent_id == agent_id:
                agent.status = "busy"
                if not hasattr(agent, 'current_load'):
                    agent.current_load = 0
                agent.current_load += 1
                break
    
    async def _release_agents(self, swarm_id: str, agent_ids: List[str]):
        """Release agents back to idle status"""
        agents = self.swarm_manager.swarm_agents.get(swarm_id, [])
        for agent in agents:
