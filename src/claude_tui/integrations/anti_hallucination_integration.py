"""
Anti-Hallucination Integration Module - Seamless integration with Claude-TUI.

Provides seamless integration between the Anti-Hallucination Engine
and the main Claude-TUI system:
...
from src.claude_tui.validation.performance_optimizer import PerformanceOptimizer
from src.claude_tui.validation.training_data_generator import TrainingDataGenerator
from src.claude_tui.validation.types import ValidationResult, ValidationSeverity
# Removed to fix circular import - will import AIInterface when needed
...
class AntiHallucinationIntegration:
    ...
    into the Claude-TUI system workflow.
    """
    ...
    async def register_ai_interface_hooks(self, ai_interface) -> None:  # Type annotation removed to avoid circular import
        """
    ...
    async def _generate_project_validation_summary(
        ...
        summary_path = Path(project.path) / '.claude-tui' / 'validation_summary.json'
