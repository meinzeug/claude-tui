...
The centerpiece of Claude-TUI's validation system, implementing:
...
class FeatureExtractor:
    """Extract features from code for ML models with memory optimization."""
    ...
    def __init__(self, cache_size_limit: int = 1000):
        ...
        self._feature_cache: Dict[str, Tuple[Dict[str, Any], float]] = {}  # hash -> (features, timestamp)
        self._cache_size_limit = cache_size_limit
        self._cache_ttl = 3600  # 1 hour TTL
        self._last_cleanup = time.time()
    
    async def extract_features(self, code: str, language: str = None) -> Dict[str, Any]:
        """Extract comprehensive features from code with caching and memory optimization."""
        # Generate cache key
        ...
        if len(self._feature_cache) < self._cache_size_limit:
            self._feature_cache[cache_key] = (features.copy(), time.time())
        
        return features
    
    async def _cleanup_feature_cache(self):
        """Clean expired entries from feature cache"""
        ...
        if len(self._feature_cache) > self._cache_size_limit:
            ...
            items_to_keep = sorted_items[-self._cache_size_limit:]
            self._feature_cache = dict(items_to_keep)
        
        # Force garbage collection if memory usage is high
        memory_percent = psutil.virtual_memory().percent
        if memory_percent > 80:
            gc.collect()
            logger.info(f"Feature cache cleanup: {len(expired_keys)} expired, memory: {memory_percent:.1f}%")
...
class AntiHallucinationEngine:
    ...
    async def train_pattern_recognition_model(self, training_data: List[CodeSample]) -> ModelMetrics:
        ...
        try:
            # Extract features in batches for performance optimization
            logger.info("Extracting features in batches...")
            X, y = await self._extract_features_batch(training_data, batch_size=50)
...
def process_data(data):
    ...
    try:
        # Process training data for pattern recognition
...
'''
    ...
    async def _extract_features_batch(
        ...
        """Extract features from training data in batches for optimal performance."""
        X = []
        y = []
        
        # Process samples in batches to optimize memory usage and performance
        for i in range(0, len(training_data), batch_size):
            batch = training_data[i:i + batch_size]
            logger.debug(f"Processing feature extraction batch {i//batch_size + 1}/{(len(training_data) + batch_size - 1)//batch_size}")
            
            # Extract features concurrently within batch
            try:
                batch_features = await self._extract_batch_features_concurrent(batch)
                
                for j, sample in enumerate(batch):
                    if j < len(batch_features) and batch_features[j]:  # Safety check
                        feature_vector = list(batch_features[j].values())
                        if feature_vector:  # Only add non-empty features
                            X.append(feature_vector)
                            y.append(1 if sample.is_authentic else 0)
                
            except Exception as e:
                logger.warning(f"Batch {i//batch_size + 1} processing failed: {e}")
                # Continue with next batch
                continue
        
        if not X:
            logger.error("No features extracted successfully")
            raise ValueError("Feature extraction failed for all samples")
        
        return np.array(X), np.array(y)
    
    async def _extract_batch_features_concurrent(
        self, 
        batch: List[CodeSample]
    ) -> List[Dict[str, Any]]:
        """Extract features concurrently for a batch to avoid blocking event loop."""
        # Use ThreadPoolExecutor to avoid blocking the asyncio event loop
        with ThreadPoolExecutor(max_workers=min(4, len(batch))) as executor:
            ...
            # Wait for all features to be extracted
            try:
                ...
                for i, features in enumerate(batch_features):
                    if isinstance(features, dict) and features:
                        ...
                        logger.warning(f"Feature extraction failed for sample {i}: {features}")
    ...
    def _extract_features_sync(self, code: str, language: str = None) -> Dict[str, Any]:
        """Optimized synchronous feature extraction for thread pool execution."""
        try:
            features = {}
            ...
            # Optimized complexity metrics with caching
            features['cyclomatic_complexity'] = self._calculate_complexity_fast(code)
            features['nesting_depth'] = self._calculate_nesting_depth_fast(code)
            
            # Pattern-based features (optimized with single regex compilation)
            features.update(self._extract_pattern_features_fast(code, language))
            
            return features
            
        except Exception as e:
            logger.debug(f"Sync feature extraction failed: {e}")
            return {}
    
    def _calculate_complexity_fast(self, code: str) -> float:
        """Fast complexity calculation with optimized regex."""
        # Use single compiled regex for better performance
        complexity_pattern = re.compile(r'\b(if|elif|else|for|while|try|except|case|switch)\b', re.IGNORECASE)
        matches = len(complexity_pattern.findall(code))
        return min(1 + matches, 50)  # Cap at 50
    
    def _calculate_nesting_depth_fast(self, code: str) -> int:
        """Fast nesting depth calculation."""
        max_depth = 0
        for line in code.split('\n')[:100]:  # Limit to first 100 lines for performance
    ...
    def _extract_pattern_features_fast(self, code: str, language: str) -> Dict[str, Any]:
        """Extract pattern-based features efficiently."""
        features = {}
        
        # Pre-compiled patterns for performance
        patterns = {
            'todo_count': re.compile(r'TODO|FIXME|HACK', re.IGNORECASE),
            'placeholder_count': re.compile(r'placeholder|implement|\.\.\.', re.IGNORECASE),
            'function_count': re.compile(r'def\s+\w+|function\s+\w+', re.IGNORECASE)
        }
        
        # Apply patterns
        for feature_name, pattern in patterns.items():
            features[feature_name] = len(pattern.findall(code))
        
        # Language-specific optimized patterns
        if language == 'python':
            features['import_count'] = len(re.findall(r'^(?:import|from)\s+\w+', code, re.MULTILINE))
            features['pass_count'] = code.count(' pass')
        elif language == 'javascript':
            features['var_declarations'] = len(re.findall(r'\b(?:var|let|const)\s+\w+', code))
            features['console_logs'] = code.count('console.log')
