...
class MemoryPool {
    ...
    checkMemoryUsage() {
        ...
        if (used.heapUsed > this.memoryThreshold) {
            setImmediate(() => console.error(`[MEMORY WARNING] Heap usage: ${Math.round(used.heapUsed / 1024 / 1024)}MB`);
            // Force garbage collection if available
            if (global.gc) {
                global.gc();
                setImmediate(() => console.error('[MEMORY] Forced garbage collection');
            }
            // Clear pools
...
process.on('SIGINT', async () => {
    setImmediate(() => console.error('Graceful shutdown initiated...');
    memoryPool.cleanup();
    process.exit(0);
});
          ...
          try {
            ...
            return {
              ...
              agents: agents.map((agent) => ({
                ...
                type: agent.type,
                status: agent.status,
                capabilities: (() => { try { return JSON.parse(agent.capabilities || '[]'); } catch(e) { return null; } })(),
        ...
        try {
          ...
          if (global.agentTracker) {
            ...
            if (status.agentCount > 0) {
              ...
              const swarm = swarmDataRaw ? (typeof swarmDataRaw === 'string' ? (() => { try { return JSON.parse(swarmDataRaw); } catch(e) { return null; } })() : swarmDataRaw) : {};
          ...
          const swarm = typeof swarmDataRaw === 'string' ? (() => { try { return JSON.parse(swarmDataRaw); } catch(e) { return null; } })() : swarmDataRaw;
          ...
          const swarmAgents = agentsData
            ...
            .map((entry) => {
              try {
                return (() => { try { return JSON.parse(entry.value); } catch(e) { return null; } })();
              } catch (e) {
                return null;
              }
            })
...
async function startMCPServer() {
  ...
  process.stdin.on('data', async (chunk) => {
    ...
    for (const line of lines) {
      if (line.trim()) {
        try {
          const message = (() => { try { return JSON.parse(line); } catch(e) { return null; } })();
