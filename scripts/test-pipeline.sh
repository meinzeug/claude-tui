...
test_docker_image() {
    ...
    if docker run --rm $DOCKER_IMAGE:$DOCKER_TAG python -c "import claude_tui; print('Health check passed')"; then
...
main() {
    ...
    if ! python -m py_compile src/claude_tui/__init__.py 2>/dev/null; then
