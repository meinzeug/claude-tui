...
DOCKER_IMAGE="claude-tui"
DOCKER_TAG="test"
K8S_NAMESPACE="claude-tui-test"

echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}Claude TUI CI/CD Pipeline End-to-End Test${NC}"
echo -e "${BLUE}=================================================${NC}"
...
test_docker_image() {
    ...
    if docker run --rm -d --name claude-tui-test $DOCKER_IMAGE:$DOCKER_TAG sleep 30; then
        print_status "Docker container started successfully"
        docker stop claude-tui-test
    else
    ...
    if docker run --rm $DOCKER_IMAGE:$DOCKER_TAG python3 -c "import claude_tui; print('Health check passed')"; then
...
test_monitoring() {
    ...
    if [[ -f "monitoring/prometheus-config.yaml" ]]; then
        ...
        if python3 -c "import yaml; yaml.safe_load(open('monitoring/prometheus-config.yaml'))"; then
    ...
    if [[ -f "monitoring/grafana-dashboard.json" ]]; then
        ...
        if python3 -c "import json; json.load(open('monitoring/grafana-dashboard.json'))"; then
...
# Claude TUI CI/CD Pipeline Test Report
...
---
Generated by: Claude TUI Pipeline Test Script
EOF
...
main() {
    ...
    if ! python3 -m py_compile src/claude_tui/__init__.py 2>/dev/null; then
